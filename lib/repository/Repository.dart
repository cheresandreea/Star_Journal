...
class Repository implements IRepository {
  ...
  Future<List<Star>> getStars() async {
    ...
    return List.generate(maps.length, (i) {
      return Star(
        id: maps[i]['id'], // Provide 0 as fallback
        name: maps[i]['name'], // Provide empty string as fallback
        radius: maps[i]['radius'] ?? 0.0, // Provide 0.0 as fallback
        xPosition: maps[i]['xPosition'] ?? 0.0, // Provide 0.0 as fallback
        yPosition: maps[i]['yPosition'] ?? 0.0, // Provide 0.0 as fallback
        temperature: maps[i]['temperature'] ?? 0, // Provide 0 as fallback
        galaxy: maps[i]['galaxy'] ?? '', // Provide empty string as fallback
        constellation: maps[i]['constellation'] ?? '', // Provide empty string as fallback
        description: maps[i]['description'] ?? '', // Provide empty string as fallback
      );
    });
  }
